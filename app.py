...
<local:MyCard Title="PCL Intelligence：主页面" Margin="0,5">
    ...
    <Grid>
      ...
      <local:MyTextBox x:Name="jumpbox" Height="30" HintText="在此输入对话内容，注意不要按下 Enter 键，这会直接启动游戏。"/>
        <local:MyButton HorizontalAlignment="right" Width="200"
            Grid.Column="1" Text="对话->" EventType="打开帮助" 
            EventData="{Binding Path=Text,ElementName=jumpbox,StringFormat='{}https://pclintelligence.19991230.xyz/Custom.json?q={0}' }"/>
    </Grid>
    <TextBlock Margin="0,10,0,0" HorizontalAlignment="right" Text="模型: Google Gemini 2.5 Flash (API)"/>
    </StackPanel>
</local:MyCard>
</StackPanel>
