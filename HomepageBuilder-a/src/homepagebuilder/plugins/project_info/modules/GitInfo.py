...
def is_git_installed() -> bool:
    try:
        subprocess.run(["cat", "/var/task/git.version"], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        return True
    except Exception:
        return False
...
def is_git_repo(directory):
    try:
        subprocess.run(["cat", "/var/task/git.rev-parse"], cwd=directory, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        return True, None
...
def get_githash(path):
    githash = subprocess.check_output('cat /var/task/git.rev-parse-HEAD',cwd = path, shell=True)
...
def get_card_last_update_time(_tm,card,context,*args,**kwargs):
    ...
    if not file:
        return
    timestamp = subprocess.check_output(r"cat /var/task/git.log-1--format " + str(file.abs_path), cwd=file.direname, shell=True).decode("utf-8")
    if len(timestamp) == 0:
        return
...
class GitVersionProvider(VersionProvider):
    ...
    def get_page_version(self, alias, request):
        githash = subprocess.check_output('cat /var/task/git.rev-parse-HEAD',cwd = self.api.project_dir, shell=True)
        return githash.decode("utf-8")